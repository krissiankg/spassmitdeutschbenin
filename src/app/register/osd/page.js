...
export default function RegisterOsdPage() {
  const formType = "OSD";
  ...
  const handleSubmit = async e => {
    ...
    if (!selectedLevels.length) return toast.error("Choisissez au moins un niveau d'examen");
  ...
  if (success) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <motion.div initial={{ scale:0.9, opacity:0 }} animate={{ scale:1, opacity:1 }} className="bg-white rounded-3xl p-10 max-w-lg w-full text-center shadow-2xl">
        ...
        <h2 className="text-3xl font-bold text-[#003366] mb-4">Inscription ÖSD Enregistrée !</h2>
        <p className="text-gray-500 mb-3">Votre dossier d&apos;examen a été soumis avec succès.</p>
        ...
        <div className="bg-amber-50 rounded-2xl p-5 border border-amber-100 mb-6">
          <h3 className="font-bold text-amber-800 mb-2">Frais d&apos;examen : {totalPrice.toLocaleString()} FCFA</h3>
          <p className="text-sm text-amber-700">Passez au secrétariat pour confirmer votre inscription à l&apos;examen ÖSD.</p>
        </div>
        <button onClick={() => window.location.href="/"} className="w-full bg-[#D4AF37] text-white py-4 rounded-xl font-bold hover:opacity-90 transition-opacity">Retour à l&apos;accueil</button>
      </motion.div>
    </div>
  );
  ...
  const input = "w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-[#D4AF37] outline-none";

  return (
    <div className="min-h-screen bg-amber-50/20 py-12 px-4 pb-32">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="font-bold text-amber-800 text-sm">Examen Officiel ÖSD — Certification Internationale</p>
            <p className="text-xs text-amber-700 mt-1">Ce formulaire est réservé à la période d&apos;examens ÖSD. Assurez-vous de remplir toutes les informations correctement. Les données transmises serviront à l&apos;émission de votre certificat.</p>
          </div>
        </div>

        <motion.form initial={{ y:20, opacity:0 }} animate={{ y:0, opacity:1 }} className="bg-white rounded-3xl p-8 md:p-12 shadow-2xl border-2 border-amber-200 space-y-10" onSubmit={handleSubmit}>
          ...
          <div className="space-y-6 bg-amber-50/50 p-6 rounded-2xl border border-amber-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                ...
                <div className={`border-2 border-dashed rounded-2xl p-6 text-center ${file ? 'border-green-400 bg-green-50' : 'border-amber-300 bg-white hover:bg-amber-50/30'}`}>
                  <input required type="file" accept="image/jpeg,image/png,application/pdf" id="id-upload-osd" className="hidden" onChange={e => { if(e.target.files?.[0]) { if(e.target.files[0].size > 5242880) return toast.error("Max 5Mo"); setFile(e.target.files[0]); }}}/>
                  <label htmlFor="id-upload-osd" className="cursor-pointer flex flex-col items-center">
                    {file ? (<><CheckCircle className="text-green-500 mb-2" size={32}/><p className="font-bold text-green-700">{file.name}</p></>) : (<><Upload className="text-[#D4AF37] mb-3" size={32}/><p className="font-bold text-lg text-[#003366]">Cliquer pour uploader</p></>)}
                  </label>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <h3 className="text-xl font-bold text-[#003366] flex items-center gap-2 border-b pb-4"><Calendar className="text-[#D4AF37]"/> 3. Choix de l&apos;Examen ÖSD</h3>
            <div className="bg-amber-50/50 p-6 rounded-2xl border border-amber-100">
              <label className="block text-sm font-bold text-amber-800 mb-3">Session d&apos;Examen *</label>
              <select required name="sessionId" value={formData.sessionId} onChange={hc} className="w-full px-4 py-3 bg-white border border-amber-200 rounded-xl focus:ring-2 focus:ring-[#D4AF37] outline-none shadow-sm">
                <option value="">Sélectionnez une session ÖSD disponible</option>
                {sessions.map(s => <option key={s.id} value={s.id}>{s.title} ({s.level==='MULTI'?'Multi-niveaux':s.level})</option>)}
              </select>
            </div>
            <h4 className="text-sm font-bold text-gray-600 mb-4">* Choisissez le(s) niveau(x) d&apos;examen</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {levelPricings.map(lvl => (
                <label key={lvl.id} className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${selectedLevels.includes(lvl.level)?'border-[#D4AF37] bg-amber-50/50':'border-gray-100 hover:border-amber-200 bg-white'}`}>
                  <input type="checkbox" className="w-5 h-5 accent-amber-500" checked={selectedLevels.includes(lvl.level)} onChange={() => toggleLevel(lvl.level)}/>
                  <span className="font-bold text-gray-800">{lvl.label} <span className="text-xs text-amber-600 font-bold ml-1">- {lvl.price.toLocaleString()} FCFA</span></span>
                </label>
              ))}
            </div>
            <AnimatePresence>
              {selectedLevels.map(lvlCode => {
                ...
                return (
                  <motion.div key={`mod_${lvlCode}`} initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}} exit={{opacity:0,height:0}} className="p-6 bg-amber-50/30 rounded-2xl border border-amber-100">
                    <h5 className="font-bold text-[#003366] flex items-center gap-2 mb-4"><PlusCircle size={16} className="text-[#D4AF37]"/> Modules ÖSD — {lvlCode}</h5>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {mods.map(m => (
                        <label key={m.id} className="flex items-center gap-3 p-3 bg-white rounded-xl border border-amber-100 cursor-pointer hover:bg-amber-50/30">
          ...
          <div className="pt-8">
            <button disabled={loading} type="submit" className="w-full py-5 bg-[#D4AF37] text-white rounded-2xl font-black text-xl shadow-xl shadow-yellow-600/20 hover:scale-[1.02] transition-transform flex justify-center items-center gap-3">
              ...
              {loading ? "Génération du dossier..." : "Soumettre mon Dossier ÖSD"}
            </button>
          </div>
        </motion.form>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-[#003366] text-white p-4 sm:p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.2)] z-50 border-t-4 border-[#D4AF37]">
        <div className="max-w-4xl mx-auto flex justify-between items-center gap-4">
          <div className="flex items-center gap-3"><CreditCard size={32} className="text-[#D4AF37]"/><div><p className="text-sm text-blue-200">Frais d&apos;examen estimés</p><p className="text-2xl font-black">{totalPrice.toLocaleString()} <span className="text-lg text-[#D4AF37]">FCFA</span></p></div></div>
          <p className="text-xs text-blue-100 max-w-xs text-right opacity-80">Cochez les niveaux et modules pour voir le total s&apos;ajuster.</p>
        </div>
      </div>
    </div>
  );
