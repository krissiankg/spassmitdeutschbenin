...
export async function POST(request) {
  try {
    ...
    // Optimisation : récupérer tous les résultats existants en une seule requête
    const existingResults = await prisma.result.findMany({
        where: { sessionId }
    });
    const existingResultMap = new Map();
    for (const r of existingResults) {
        existingResultMap.set(r.candidateId, r);
    }

    const operations = results.map((res) => {
        const { candidateId, scores, total, decision } = res;
        const existingResult = existingResultMap.get(candidateId);
